...
router.get('/', requireLogin, requirePermission('smsTest'), async (req, res) => {
  try {
...
router.post('/send', requireLogin, requirePermission('smsTest'), async (req, res) => {
  try {
    const { number, message } = req.body;

    if (!number || !message) {
      return res.redirect('/sms-test');
    }
