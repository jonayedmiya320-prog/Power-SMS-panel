...
router.get('/', requireLogin, requirePermission('creditNotes'), async (req, res) => {
  try {
    ...
    res.render('creditnotes', {
      ...
      notes: [],
      clients: [],
      error: 'Could not load credit notes.'
    });
  }
});

router.post('/create', requireLogin, requirePermission('creditNotes'), async (req, res) => {
  try {
    const { clientName, amount, reason } = req.body;

    if (!clientName || !amount || !reason) {
      return res.redirect('/credit-notes');
    }
...
router.post('/delete/:id', requireLogin, requirePermission('creditNotes'), async (req, res) => {
  try {
